...
<head>
    ...
    <title>Peneriman Non Medis - Print</title>
    <style>
        body {
    ...
    <div class="container">
        ...
        <?php foreach ($penerimaan_nonmedis_con as $dt_penerimaan_nonmedis) : ?>
            <table class="details">
                <tr>
                    ...
                    <td>Tgl. Datang</td>
                    <td>:</td>
                    <td><?= date('d-M-Y', strtotime($dt_penerimaan_nonmedis['tgl_pesan'])) ?></td>
                </tr>
                <tr>
                    <!-- untuk yang kiri -->
                    <td>No. Order</td>
                    ...
                    <td>Tgl. Beli</td>
                    <td>:</td>
                    <td><?= date('d-M-Y', strtotime($dt_penerimaan_nonmedis['tgl_faktur'])) ?></td>
                </tr>
                <tr>
                    <!-- untuk yang kiri -->
                    <td>Supplier</td>
                    ...
                    <td>Jatuh Tempo</td>
                    <td>:</td>
                    <td><?= date('d-M-Y', strtotime($dt_penerimaan_nonmedis['tgl_tempo'])) ?></td>
                </tr>
                <tr>
                    <!-- untuk yang kiri -->
                    <td>Petugas</td>
                    <td>:</td>
                    <td style="padding-right: 30px"><?= $dt_penerimaan_nonmedis['nama'] ?></td>
                    <!-- untuk yang kanan -->
                </tr>
            </table>
        <?php endforeach ?>
        ...
        <table class="table">
            ...
            <tbody>
                <?php if (!empty($penerimaan_nonmedis_det_con)) : ?>
                    ...
                    <?php foreach ($penerimaan_nonmedis_det_con as $index => $dt_penerimaan_nonmedis_det) : ?>
                        <tr style=" white-space: nowrap;">
                            ...
                            <td>Rp <?= number_format($dt_penerimaan_nonmedis_det['total'], 0, ',', '.'); ?></td>
                        </tr>

                        <!-- menghitung sum jumlah -->
                        <?php
                        $total_jumlah += $dt_penerimaan_nonmedis_det['jumlah'] ?>
                    <?php endforeach; ?>
                <?php else : ?>
                    <tr>
                ...
                <?php endif; ?>
                <?php foreach ($penerimaan_nonmedis_con as $dt_penerimaan_nonmedis) : ?>
                    <?php if ($dt_penerimaan_nonmedis['total1']) : ?>
                        <tr>
                            <td colspan="7"></td>
                            <td><?= $total_jumlah; ?></td>
                            <td>Rp <?= number_format($dt_penerimaan_nonmedis['total1'], 0, ',', '.'); ?></td>
                        </tr>
                    <?php endif ?>
                <?php endforeach ?>
            </tbody>
